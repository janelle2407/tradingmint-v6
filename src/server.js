...
const { readDb, writeDb, resetDb, addJournal } = require("./storage/db"); const { DEFAULT_SYMBOLS, fetchUniverse, round, percentMove, fetchEarningsCalendar } = require("./data/marketData"); const { scanMarket } = require("./engines/scanner"); const { applyTradeFilters } = require("./engines/filters"); const { enterPaper, exitPaper, updateOpenPositions, paperStats } = require("./engines/paper"); const { runPortfolioBacktest, optimize } = require("./engines/backtest"); const { brokerStatus, placeOrder } = require("./broker/adapter");
...
const app=express(); const PORT=process.env.PORT||10000; const VERSION="6.8.0-earnings-calendar-detailed-reasons"; app.use(express.json({limit:"1mb"})); app.use(express.static(path.join(__dirname,"../public"))); let lastUniverse=null,lastUniverseTime=0;
