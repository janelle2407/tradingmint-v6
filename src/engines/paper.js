...
const exposure = signal
? exposureBlocked(signal, db.paper, db.settings, barsBySymbol)
: { blocked: false };

if (exposure.blocked) {
addJournal(db, "ENTRY_REJECTED_EXPOSURE", signal?.symbol || "-", exposure.reason, {
signal,
exposure
});
...
if (!check.ok) {
addJournal(db, "ENTRY_REJECTED", signal?.symbol || "-", check.reasons.join(" "), {
signal
});
...
// Recalculate stop/targets from the actual entry price.
// This prevents stop/target levels being based on yesterday's close if entry cost shifts.
...
? round(Number(signal.trailingStop) + priceDiff)
: adjustedStop;
...
const adjustedExit = applyCosts(
Number(exitPrice || position.lastPrice || position.entry),
"sell",
db.settings
);
...
"PARTIAL_EXIT",
position.symbol,
`${position.symbol} partial exit at T1, stop moved to breakeven`,
partial
);
