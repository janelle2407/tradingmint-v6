const { round } = require("../data/marketData");
...
function enterPaper(db, signal, source = "manual", barsBySymbol = {}) {
  ...
  const entry = applyCosts(Number(signal.entry), "buy", db.settings);
  const shares = check.shares;
  const cost = entry * shares + Number(db.settings.commissionPerTrade || 0);
  if (cost > db.paper.cash) return { ok: false, reasons: ["Not enough paper cash."] };
  ...
  const trailingAtr = Number(signal.trailingAtr || originalRisk * 0.5);
  ...
  const position = {
    ...
    baseQualityScore: signal.baseQualityScore || null,
    rsPercentile: signal.rs?.rsPercentile || null,
    volumeRatio: signal.volumeRatio || null,
    marketRegime: signal.marketRegime || signal.regime || null,
...
function exitPaper(db, positionId, exitPrice, reason = "Manual exit") {
  ...
  const closed = { ...position, exit: Number(adjustedExit.toFixed(4)), exitTime: new Date().toISOString(), exitReason: reason, proceeds: Number(proceeds.toFixed(2)), pnl: Number(pnl.toFixed(2)), pnlPct: Number(pnlPct.toFixed(2)) };
...
function partialExitPaper(db, position, exitPrice, reason) {
  ...
  const pnl = proceeds - cost;
  const partial = { ...position, shares: halfShares, exit: Number(adjustedExit.toFixed(4)), exitTime: new Date().toISOString(), exitReason: reason + " (partial, half position)", proceeds: Number(proceeds.toFixed(2)), pnl: Number(pnl.toFixed(2)), pnlPct: Number(((pnl / cost) * 100).toFixed(2)), isPartial: true };
  ...
  addJournal(db, "PARTIAL_EXIT", position.symbol, `${position.symbol} sold half at ${money(adjustedExit)}, stop moved to breakeven ${money(position.entry)}. P/L: ${money(pnl)}`, { partial, remainingShares: position.shares, newStop: position.entry });
...
function updateOpenPositions(db, signals) {
  ...
  for (const position of [...db.paper.open]) {
    ...
    else if (signal.price >= position.target2 && position.partialExitDone) exits.push(exitPaper(db, position.id, signal.price, "Target 2 hit, full exit"));
    else if (signal.price >= position.target1 && !position.partialExitDone && position.shares >= 2) partialExitPaper(db, position, signal.price, "Target 1 hit");
    else if (signal.price >= position.target1 && !position.partialExitDone && position.shares < 2) exits.push(exitPaper(db, position.id, signal.price, "Target 1 hit"));
    else if (signal.safety === "REJECT" && signal.confidence < 55) exits.push(exitPaper(db, position.id, signal.price, "Signal deteriorated"));
  }
  return exits;
}
...
function paperStats(db) {
  ...
  if (!closed.length) return { totalTrades: 0, winRate: null, expectancy: null, profitFactor: null, maxDrawdown: null, totalReturn: null, equityCurve: [] };
  ...
  return { totalTrades: closed.length, winRate: Number(((wins.length / closed.length) * 100).toFixed(1)), expectancy: Number(expectancy.toFixed(2)), profitFactor: grossLosses ? Number((grossWins / grossLosses).toFixed(2)) : null, maxDrawdown: Number(maxDrawdown.toFixed(2)), totalReturn: Number(((totalPnl / Number(db.settings.startingCash || 5000)) * 100).toFixed(2)), equityCurve };
}
